...
with open(election_write, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        county_results = (f"{county_name}: {county_percentage:.1f}%  ({cty_votes:,})\n")

         # 6e: Save the county votes to a text file.
        print(county_results)
        txt_file.write(county_results)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (cty_votes > Votes_turnout):
            Votes_turnout = cty_votes
            large_turnout_county = county_name
            


    # 7: Print the county with the largest turnout to the terminal.
    print (large_turnout_county)

    # 8: Save the county with the largest turnout to a text file.
    txt_file.write(
        f'\n------------------------------\n'
        f' Largest County Turnout:{large_turnout_county}\n'
        f'----------------------------------\n')
